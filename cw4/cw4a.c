...
#define handle_error(msg)   \
    ...
    // Modify sleep based on environmental variable MODIFY_SLEEP
    char modify_sleep = !!getenv("MODIFY_SLEEP");
    ...
    if (modify_sleep)
    {
        ...
        if (timerfd < 0)
            handle_error("Timerfd: error while creating!");

        struct itimerspec new_value;

        new_value.it_interval.tv_sec = 0;
        new_value.it_interval.tv_nsec = udelsmp * 1000;
    ...
    for (i = 0; i < nsmp; i++)
    {
        ...
        if (modify_sleep)
        {
            ...
            if (s != sizeof(uint64_t) || exp != 1)
                handle_error("Timerfd: error while reading!");
        }
        else
        {
            usleep(udelsmp);
